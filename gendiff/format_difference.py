  ...
  def inner(diff, depth=0):
    ...
    for key in keys:
      if diff[key]['condition'] == 'deleted':
        difference.append(f'{indent}- {key}: {inner(diff[key]["value"], depth)}')
        ...
        difference.append(f'{indent}- {key}: {inner(diff[key]["value1"], depth)}')
        difference.append(f'{indent}+ {key}: {inner(diff[key]["value2"], depth)}')

      elif diff[key]['condition'] == 'changed':
        difference.append(f'{indent}  {key}: {inner(diff[key]["children"], depth)}')
    for i in range(depth):
        depth -= 1
        difference.append('\t' * depth + '}' + '\n')
